...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    #If the ghost is in jail, then set the value of position of
    #the jail to be one. And then, update the probability.
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    pacmanPosition = gameState.getPacmanPosition()
    "*** YOUR CODE HERE ***"
    #Check if the ghost has been captured.
    if noisyDistance == None:
        jailCounter = util.Counter()
        jailCounter[self.getJailPosition()] = 1.0
        self.beliefs = jailCounter
        ...
        for p in self.particles:
            ...
            #Update the Pr(trueDistance/noisyDistance) 
        self.beliefs = tempCounter
...
class JointParticleFilter:
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
        ...
        #For every ghost in this particl
        for i in range(self.numGhosts):
          newPosition = getPositionDistributionForGhost(setGhostPositions(gameState, prevGhostPositions), i, self.ghostAgents[i])
